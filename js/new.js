...
function addStoriesToLayer(id,locations,val) {
    var markers = L.markerClusterGroup();
    locations.map(item => L.marker(new L.LatLng(item.lat, item.lng)))
        .forEach(item => markers.addLayer(item));
...
function GetCluster(story_id) {
    ...
    $.get(appUrl, parameter, function (data) {
        ...
        addStoriesToLayer(story_id,gps_locations,true)
            x.addEventListener('click',function(){
                addStoriesToLayer(story_id,gps_locations,x.checked)
            })
...
mymap = L.map('map', {
  ...
  layers: [streets, markers]
    ...
    genInput.addEventListener('click', function () {
        ...
        // } else if(genInput.checked === false) {
        //     markers.eachLayer(function (layer) {
        //         layer.remove()
        //     })
        //     mymap.removeLayer(markers);
